...
model.prototype.create = function(post){
  //通过
  return new Promise((resolve,reject)=>{
    this.data = post_parse(post);
    if(!this.data){
      return reject('解析post参数失败');
    }

    this.validation().then(()=>resolve(this),err=>reject(err))});
};
...
model.prototype.add  = function(){
  this.link().then((cli)=>{
    ...
    this.pool.releaseConnection(cli);
    throw new Error( `数据库连接池错误${err.stack}`,);
  });
};

///////////////////////////mysql pool Promise 操作////////////////////////
